...
function sports_blog_fonts_url() {
	...
	if ('off' !== _x('on', 'Source Sans Pro font: on or off', 'sports-blog')) {
		$fonts[] = 'Source+Sans+Pro:300,300i,400,400i,700,700i';
	}

	/* translators: If there are characters in your language that are not supported by Poiret One, translate this to 'off'. Do not translate into your own language. */
	if ('off' !== _x('on', 'Poiret One font: on or off', 'sports-blog')) {
		$fonts[] = 'Poiret+One';
	}
...
function sports_blog_scripts() {
	...
	if (!empty($fonts_url)) {
		wp_enqueue_style('sports-blog-google-fonts', $fonts_url, array(), null);
	}
